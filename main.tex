...
\end{equation}
\subsubsection{Representation A}
Stress and internal forces
\begin{equation}
  \sigma=\epartd{\psi}{\veps} \AND \beta_i = -\epartd{\psi}{\alpha_i}
\end{equation}
\begin{align}
  \nonumber  \sigma &= E_0\,\veps + \suml{i=1}{n}E_i\rbr{\veps-\alpha_i} \\
  \beta_i &= E_i \rbr{\veps-\alpha_i}\label{eq:repa1}
\end{align}
\emph{Dissipation potential}, a smooth quadratic function:
\begin{equation}
\textstyle \phi^*(\veci{\beta}) = \suml{i=1}{n} \half\frac{1}{\eta_i}\beta_i^2
\end{equation}
%
yields the evolution
%
\begin{align}
  \dot{\alpha}_i &= \epartd{\phi^*}{\beta}\quad\textup{in the closed form} \\
  \dot{\alpha}_i &= \frac{1}{\eta_i}\beta_i \COM \alpha_i\rbr{0} = 0
\end{align}

\subsubsection{Representation B (Reduced Representation)}

Strain--like internal variables are removed by (\ref{eq:repa1})\textsubscript{2}
and the viewpoint is restricted to internal forces:
%
\begin{equation}
  \sigma = E_0\,\veps+\suml{i=1}{n}\beta_i
\end{equation}
%
\begin{equation}
  \textstyle \dot{\beta}_i=\linedot{E_i\rbr{\veps-\alpha_i}} =
  E_i\rbr{\dot{\veps}\frac{1}{\eta_i}\beta_i}
\end{equation}
yields
\begin{equation}
  \dot{\beta}_i+\frac{1}{\tau_i}\beta_i = E_i\dot{\veps} \WITH \tau_i=\frac{\eta_i}{E_i}
\end{equation}


\subsubsection{Representation C (Convolution Representation)}
\begin{align}
  &\dot{\alpha}_i=\frac{1}{\eta_i}E_i\rbr{\veps-\alpha_i} =
  \frac{1}{\tau_i}\rbr{\veps-\alpha_i} \\
  \Rightarrow\quad &\dot{\alpha}_i+\frac{1}{\tau_i}\alpha_i=\frac{1}{\tau_i}\veps
\end{align}
This evolution equation admits an \emph{integration factor} $\exp\rbr{t/\tau_i}$,
i.e
\begin{equation}
  \ddt\sbr{\exp\rbr{t/\tau_i}\alpha_o(t)} = \frac{1}{\tau_i}\exp\rbr{t/\tau_i}\veps(t)
\end{equation}
...
The split is applied same as in (\ref{eq:split1}). In view to the modeling of
polymers, we consider (almost incompressible) volumetric part elastic and (very
soft) isochoric part viscoelastic. The 1d setting is applied to the isochoric
part, which is considered to be initially isotropic.
\begin{align}
  \nonumber \psi(\Bveps,\veci{\Balpha}) &= \half\kappa e^2 + \mu_0\,\Bveps':\Bveps'
  + \suml{i=1}{n}\mu_i\Abs{\Bveps'-\Balpha_i}^2\\
   \phi^*(\veci{\Bbeta}) &=
  \suml{i=1}{n}\half\textstyle\frac{1}{\eta_i}\Abs{\Bbeta_i}^2
\end{align}
\begin{equation}
  \Bsigma=\epartd{\psi}{\Bveps}\SEMI\Bbeta=-\epartd{\psi}{\Balpha}\SEMI
  \dot{\Balpha}=\epartd{\phi^*}{\Bbeta}
\end{equation}

\subsubsection{Representation A}
\begin{align}
  \nonumber \Bsigma &= \kappa e \Vone +
                      \rbr{2\mu_i\,\Bveps' +
                      \suml{i=1}{n} 2\mu_i\rbr{\Bveps'-\Balpha_i}}:\BBP \\
  \nonumber \Bbeta_i &= 2 \mu_i \rbr{\Bveps'-\Balpha_i} \\
  \dot{\Balpha}_i &= \textstyle\frac{1}{\eta_i}\Bbeta_i
\end{align}

\subsubsection{Representation B}
Convenient for experimental identification where both equilibrium stress and
overstress are measured.
\begin{align}
  \nonumber \Bsigma &=
                      \underbrace{\kappa e \Vone +
                      2\mu_i\,\Bveps}_\text{eqm. stress}' +
                      \underbrace{\textstyle\suml{i=1}{n} \Bbeta_i}_\text{viscous overstress} \\
  \dot{\Bbeta}_i &+ \frac{1}{\tau_i}\Bbeta_i = 2 \mu_i \dot{\Bveps}'
\end{align}


\subsubsection{Representation C}
\begin{equation}
  \Bsigma(t)=\kappa e \Vone + \int_0^t 2
  \widehat{\mu}(t-s)\dev\rbr{\dot{\Bveps}(s)}\Ud s
\end{equation}
with shear modulus relaxation function
\begin{equation}
  \textstyle \widehat{\mu}(t)=\mu_0+\suml{i=1}{n}\mu_i\exp\rbr{- \frac{(t-s)}{\tau_i}}
\end{equation}
The integral form with the relaxation function is called \emph{convolution
  representation of viscoelasticity}. The relaxation contains all material
parameters for the isochoric part. Total number of material parameters are:
\begin{equation}
  \CM := \cbr{\kappa, \mu_0, \mu_1, \tau_1, \ldots,\mu_n,\tau_n} \AND N = 2n+2
\end{equation}
...
\begin{equation}
  \textup{State}(\Vx) := \cbr{\Bveps\,\textup{(external)}, \Bveps^p\, \textup{(internal)}}
\end{equation}
where $\Bveps^p\in\IR^6$ is a symmetric rank 2 tensor variable denoted as
plastic strain.
\subsubsection{Elastic Stress Response}
Definition of the elastic strain $\Bveps^e$ in the context of small
deformations:
\begin{equation}
  \Bveps^e := \Bveps - \Bveps^p
\end{equation}

\begin{equation}
  \psi=\widetilde{\psi}(\Bveps,\Bveps^p)=\widehat{\psi}(\Bveps-\Bveps^p)=\widehat{\psi}(\Bveps^e)
\end{equation}
%
$\widetilde{\psi}$ is assumed to be a convex function which satisfies the
normalization conditions ($\widehat{\psi}(\Bveps^e=0)=0$ and
$\epartd{\widehat{\psi}}{\Bveps}(\Bveps^e=0)=0$)
